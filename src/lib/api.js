...
export async function listMyClasses(role, userId) {
  let q = supabase.from('classes').select('*, courses(name), profiles!classes_teacher_id_fkey(full_name)');
...
export async function listAllClasses() {
  const { data, error } = await supabase.from('classes')
    .select('*, courses(name), profiles!classes_teacher_id_fkey(full_name)')
...
export async function getClass(id) {
  const { data, error } = await supabase.from('classes')
    .select('*, courses(*, lessons(*)), profiles!classes_teacher_id_fkey(full_name)')
    .eq('id', id).single();
  if (error) throw error;
  return data;
}
...
export async function listClassRoster(classId) {
  const { data, error } = await supabase.from('enrollments')
    .select('id, joined_at, profiles(id, full_name)').eq('class_id', classId);
  if (error) throw error;
  return data;
}
...
export async function listMyEnrolledClasses(studentId) {
  const { data, error } = await supabase.from('enrollments')
    .select('id, joined_at, classes(*, courses(*, lessons(*)), profiles!classes_teacher_id_fkey(full_name))')
...
export async function listLessonProgressForCourse(courseTeacherId, lessonIds) {
  ...
  const { data, error } = await supabase.from('lesson_progress')
    .select('*, profiles(full_name)').in('lesson_id', lessonIds);
  if (error) throw error;
  return data;
}
...
export async function uploadLessonVideo(userId, blob, filenameHint = 'bai-giang') {
  ...
  const { error } = await supabase.storage.from('lesson-videos').upload(path, blob, {
